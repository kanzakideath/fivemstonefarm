...
Assert-SourcePattern `
    'StartMining\(startMode\s*:=\s*"",\s*\*\)[\s\S]{0,700}TryClaimStartOperation\(&startToken\)[\s\S]{0,1000}PrepareMetagameForNewFarmStart\(\)[\s\S]{0,180}IsStartOperationCurrent\(startToken\)[\s\S]{0,5000}RunBackgroundBridge\("health"\)[\s\S]{0,180}IsStartOperationCurrent\(startToken\)' `
