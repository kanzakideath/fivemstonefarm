...
Write-Output $text
if ($p.ExitCode -ne 0 -or $text -notmatch '^STATIONARY_WAIT_PASS') {
    throw "Stationary production-loop regression failed (exit=$($p.ExitCode), evidenceMatch=$($text -match '^STATIONARY_WAIT_PASS'))."
}
