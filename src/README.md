...
FiveMが前面のときは、採掘・石洗い・砂金採りの実行前と対象喪失時にWin32 `SendInput` の相対マウス入力で下向き視点を復旧します。別アプリが前面の間は入力せず `WAIT_FG` とし、作業ボタンの再検出後だけ `TARGET_OK` として閉ループ確認します。食事はFiveMが前面なら空腹ゲージを複数フレーム確認し、裏画面でゲージを安全に読めない間だけ時間上限のフォールバックを使います。設定画面のスロットはox_inventory標準の1～5に限定し、使用前後の同スロット品の個数減少を確認できた場合だけ成功として記録します。3回連続で確認できない場合は安全停止します。
