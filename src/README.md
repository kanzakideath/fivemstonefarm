...
- `endless-wash.ahk`: 荷台前エンドレス石洗いの開始条件、復旧レシート検証、回数制限付き再始動と安全停止を担当します。
...
通常の石掘り・砂金採りは、自動収納OFFなら明示的な `work-only` bridge policyを使います。この経路は荷台表示だけを省略し、server epoch、対象ラベル、進捗IDLE、inventory CLOSEDの確認を維持します。自動収納ONと荷台前エンドレス石洗いはstorage gateを維持します。荷台検出は固有3ラベルを一般的な「インベントリを開く」より優先し、固有候補0件時だけ一般ラベルへfallbackします。

専用の荷台前エンドレス石洗いでは、実報酬後に `WASH_SETTLING` で1.15秒待ち、`recover-wash-zone` が1回80msの必須W補正と必要に応じた下向き・W/S探索を行います。その後は「荷台だけ」ならW、「洗浄だけ」ならSへ50msずつ補正し、各入力後の実測状態で次の方向を決め直します。各入力前後にepoch、進捗IDLE、inventory CLOSED、洗浄と荷台の構造を確認し、3回連続で安定した場合だけ次の `try-washing` へ戻ります。通常モードの旧補正経路とは分離しています。
