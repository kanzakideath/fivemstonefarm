...
石洗いと砂金は各操作の直前に下向き入力をクランプまで送り、実対象を再観測してからクリックします。石洗いは実報酬を確認すると `WASH_SETTLING` で既定1.2秒を無入力で待ち、アニメーション後退が止まってから `WASH_CORRECTING` で前進補正を1度だけ実行します。`WASH_VERIFYING` は補正後の対象を再観測し、見失った場合だけ追加の視点補正を1度送り、通常Recoveryへ引き継ぎます。同じボタンが複数表示された場合は、選択状態、照準との距離、DOM順で使用可能な候補を1つに決定します。
