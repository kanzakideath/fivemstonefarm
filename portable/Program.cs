...
namespace FishingPilot {
 ...
 public sealed class MainForm:Form {
  ...
  Engine engine;Options config;Label phase,detail,numbers,ring,meters;CheckBox observe,needs,background;NumericUpDown food,drink,reserve,radius;
  Button start,stop,save;NotifyIcon tray;bool closing;int countdown;System.Windows.Forms.Timer timer;
  public MainForm(){
   Text="釣りアシスト 0.2.0-preview";Width=840;Height=850;MinimumSize=new Size(660,640);StartPosition=FormStartPosition.CenterScreen;Font=new Font("Yu Gothic UI",11);BackColor=Color.FromArgb(242,244,248);
   ...
   background=new CheckBox{Text="バックグラウンド動作（対応NUIへ入力・他アプリへキーを送りません）",AutoSize=true,Checked=config.BackgroundMode};flow.Controls.Add(background);
  ...
  void Save(){if(engine.Running)throw new InvalidOperationException("設定変更はF9で停止してから行ってください");config.FoodKey=(int)food.Value;config.DrinkKey=(int)drink.Value;config.ReserveGrams=(int)reserve.Value;config.AutoNeeds=needs.Checked;config.ObserveOnly=observe.Checked;config.BackgroundMode=background.Checked;config.GaugeRadius=(double)radius.Value;string p=Path.Combine(root,"settings.json"),tmp=p+".tmp";File.WriteAllText(tmp,new JavaScriptSerializer().Serialize(config));if(File.Exists(p))File.Replace(tmp,p,p+".bak");else File.Move(tmp,p);}
  void StartNow(){try{Save();if(config.BackgroundMode&&config.AutoNeeds&&(config.FoodKey<1||config.FoodKey>5||config.DrinkKey<1||config.DrinkKey>5))throw new InvalidOperationException("裏画面の補給は登録ホットバー1〜5を使います");if(config.AutoNeeds&&(config.FoodKey==2||config.DrinkKey==2))throw new InvalidOperationException("食料・飲料キーに2は使えません。2には釣り竿をセットしてください");engine.Start(config);start.Enabled=false;}catch(Exception e){start.Enabled=true;phase.Text="開始できませんでした";detail.Text=e.Message;}}
  void OnState(Status s){if(closing||IsDisposed)return;try{BeginInvoke((Action)delegate{phase.Text=s.Phase;detail.Text=s.Detail;ring.Text=s.Ring;meters.Text="重量 "+s.Inventory+"    空腹 "+s.Hunger+"    水分 "+s.Thirst;numbers.Text="投竿 "+s.Casts+"    判定入力 "+s.Keys+"    取得確認 "+s.Results;start.Enabled=!s.Running;save.Enabled=food.Enabled=drink.Enabled=reserve.Enabled=observe.Enabled=needs.Enabled=background.Enabled=radius.Enabled=!s.Running;});}catch(InvalidOperationException){}}
