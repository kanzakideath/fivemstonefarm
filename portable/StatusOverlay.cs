...
namespace FishingPilot {
 ...
 public sealed class StatusOverlay:Form {
  Status status=new Status();IntPtr game;bool enabled=true;readonly Timer follow;OverlayOptions settings=new OverlayOptions();bool rendering;string appTheme="dark";bool Dark{get{return Runtime064.Dark(settings.Theme=="match"?appTheme:settings.Theme);}}
  Color Ink{get{return Dark?Color.FromArgb(241,242,247):Color.FromArgb(31,41,57);}}
  Color Muted{get{return Dark?Color.FromArgb(172,179,194):Color.FromArgb(103,112,128);}}
  Color Accent{get{return Dark?Color.FromArgb(102,170,255):Color.FromArgb(0,112,235);}}
  ...
  public void Configure(OverlayOptions o,string theme="dark"){if(Object.ReferenceEquals(o,settings)&&theme==appTheme)return;appTheme=theme;settings=o??new OverlayOptions();settings.Validate();Follow();Render();}
  public void UpdateState(Status s,IntPtr hwnd,bool show){status=s??new Status();game=hwnd;enabled=show;Follow();Render();}
  int DesiredHeight(){int line=settings.FontSize+18;return 45+(settings.ShowSession?92:0)+(settings.ShowHeldValue?line:0)+(settings.ShowTrunkValue?line+22:0)+(settings.ShowWeight?line:0)+(settings.ShowNeeds?line+(String.IsNullOrEmpty(status.Needs)?0:22):0)+(settings.ShowStatus?line+42:0)+(settings.ShowMovement?line:0)+(settings.ShowLifetime?line:0)+(settings.ShowDiagnostics?line*2:0)+(settings.ShowItems?Math.Min(settings.MaxRows,(status.Live.HeldValue.Items??new ItemDisplay[0]).Length)*line:0);}
  ...
  void DrawPanel(Graphics g,int width,int height){
   ...
   using(var path=Rounded(1,1,width-2,height-2,18))using(var bg=new SolidBrush(Color.FromArgb(settings.Opacity*255/100,Dark?Color.FromArgb(24,26,33):Color.FromArgb(247,249,252))))using(var border=new Pen(Color.FromArgb(Dark?45:150,255,255,255))) {g.FillPath(bg,path);g.DrawPath(border,path);}
   int y=14;var live=status.Live??new LivePanel();
   DrawText(g,"FISHING   ·   F5 / F6",9,FontStyle.Bold,Muted,ref y,23,width);
   if(settings.ShowSession){DrawText(g,"今回の釣果 · 売却見込",settings.FontSize,FontStyle.Regular,Muted,ref y,22,width);
    DrawText(g,live.SessionValue.Known?"¥"+live.SessionValue.Total.ToString("N0"):"¥0",27,FontStyle.Bold,Accent,ref y,45,width);
    DrawText(g,live.SessionValue.UnknownKinds>0?"単価未登録 "+live.SessionValue.UnknownKinds+"種（合計には未算入）":"開始前の所持分・収納による移動は含めません",9,FontStyle.Regular,Muted,ref y,25,width);}
   if(settings.ShowHeldValue)Row(g,"手持ち",Money(live.HeldValue)+(live.Fresh?"":" · 前回値"),ref y,width);
   if(settings.ShowTrunkValue){Row(g,"荷台全体",Money(live.TrunkValue),ref y,width);DrawText(g,live.TrunkKnown?(live.TrunkOpen?"開いている荷台 · ":"最終確認 "+live.TrunkUpdated+" · ")+(live.TrunkLabel==""?live.TrunkId:live.TrunkLabel):"荷台を開くと中身全体を集計します",9,FontStyle.Regular,Muted,ref y,22,width);}
   if(settings.ShowWeight)Row(g,"重量",live.Weight+(live.Fresh?"":" · 更新待ち"),ref y,width);
   if(settings.ShowNeeds){Row(g,"空腹 / 水分",status.Hunger+" / "+status.Thirst,ref y,width);if(!String.IsNullOrEmpty(status.Needs))DrawText(g,status.Needs,9,FontStyle.Regular,Muted,ref y,22,width);}
   if(settings.ShowStatus){Row(g,status.Running?"実行中":"停止",status.Phase,ref y,width);using(var f=new Font("Yu Gothic UI",9))using(var b=new SolidBrush(Muted))using(var fmt=new StringFormat{Trimming=StringTrimming.EllipsisCharacter})g.DrawString(status.Detail??"",f,b,new RectangleF(17,y,width-34,40),fmt);y+=42;}
  ...
  void Row(Graphics g,string label,string value,ref int y,int width){int h=settings.FontSize+18;if(y+h>Height-8)return;
   using(var pen=new Pen(Color.FromArgb(Dark?45:26,130,142,164)))g.DrawLine(pen,17,y,width-17,y);
   using(var f=new Font("Yu Gothic UI",settings.FontSize))using(var fg=new SolidBrush(Ink))using(var sub=new SolidBrush(Muted))using(var format=new StringFormat{Trimming=StringTrimming.EllipsisCharacter,FormatFlags=StringFormatFlags.NoWrap,LineAlignment=StringAlignment.Center}){
