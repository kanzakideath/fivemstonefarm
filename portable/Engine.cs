...
namespace FishingPilot {
 ...
 public sealed class Engine : IDisposable {
  ...
  int castRetries,consolePort;CdpBridge.FishingConnection sceneLink;
  ...
  public void Start(Options o) {
   ...
   telemetry=new Telemetry();state=new Status{Running=true};Log("start","version=0.2.0-preview observe_only="+o.ObserveOnly+" background="+o.BackgroundMode+" console="+consolePort+" auto_needs="+o.AutoNeeds);
  ...
  void Loop(CancellationToken token) {
   ...
   try {
    ...
    while(!token.IsCancellationRequested) {
     ...
     if(epoch!=""&&now>=sceneRetry)try {
      ...
      scene=sceneLink.Poll().GetAwaiter().GetResult();sceneKnown=true;
     ...
     if(!r.Valid&&!scene.Source.Contains("ambiguous")) {
      if(options.BackgroundMode&&sceneKnown&&sceneLink!=null)try{using(var b=sceneLink.Capture(scene,false).GetAwaiter().GetResult()){r=reader.ReadAuto(b);scene.Source="NUI capture";}if(r.Valid&&scene.Key>=0&&r.Key!=scene.Key)r.Valid=false;}catch(Exception e){LogThrottled("capture_unavailable",e.GetType().Name);}
      else if(foreground&&c.Width>=300&&c.Height>=240){int side=(int)Math.Min(c.Width,c.Height*.4);using(var b=Native.Capture(new Rectangle(c.X+c.Width/2-side/2,c.Y+c.Height/2-side/2,side,side),320,320))r=reader.ReadAuto(b);scene.Source="foreground capture";}
     }
     scene.Cost=Now-captureAt;scene.At=Now;now=Now;bool present=scene.Present||r.Valid;
     ...
     if(present&&!castIssued&&fresh&&pendingNeed==""){before=t.Inventory;outcomes.Clear();castIssued=true;seenRing=true;acknowledged=true;castAt=now;round.Reset();Log("adopt_existing_round","no_rod_key_sent=1");}
     ...
     if(r.Valid&&castIssued){seenRing=true;phase="challenge";reason="追加判定も継続して監視しています";
      if(fresh&&scene.Cost<180){int key=round.Observe(r,now);if(key>=0){bool sent=false,uncertain=false;
        ...
        else if(foreground)sent=Native.Press(target,targetPid,key,token);
      ...
      }}else reason="接続または描画の新鮮さを再確認しています";
     ...
     if(!present&&!castIssued) {
      ...
      if(!fresh){phase="telemetry";reason=t.Error==""?"新しい所持品データを確認中":t.Error;}
       ...
       if(needReady&&RecastPolicy.Ready(now,nextCastAt,quietSince,present,scene.Busy,fresh&&now-t.At<2500)&&!token.IsCancellationRequested){
        ...
        if(PressSlot(options.RodKey,token,foreground)){castAt=Now;castIssued=true;phase="bite";state.Casts++;Log("cast","key="+options.RodKey+" ack=pending backend="+(options.BackgroundMode&&consolePort!=0?"registered_hotbar":"foreground"));}
     ...
     Emit(t,scene,Now,foreground);token.WaitHandle.WaitOne(r.Valid?8:25);
    }
   ...
   finally{stop.Cancel();if(sceneLink!=null){sceneLink.Dispose();sceneLink=null;}state.Running=false;state.Phase="停止中";Publish();Log("stop","input_scope_closed=1");Native.timeEndPeriod(1);}
  }
  bool PressSlot(int key,CancellationToken token,bool foreground){if(options.BackgroundMode&&consolePort!=0)return CdpBridge.FishingConnection.Hotbar(consolePort,key,token);return foreground&&Native.Press(target,targetPid,key,token);}
