...
internal static partial class CdpBridge
{
    ...
    private static JavaScriptSerializer Json { get { return new JavaScriptSerializer { MaxJsonLength = 8 * 1024 * 1024 }; } }
    ...
    private sealed class CdpSession : IDisposable
    {
        ...
        public void FishingDeadline(int milliseconds) { _timeout.CancelAfter(milliseconds); }
        public async Task<Dictionary<string, object>> FishingFrames() {
            var response = await CommandAsync(_socket, "Page.getFrameTree", null, _timeout.Token).ConfigureAwait(false);
            return GetObject(GetObject(response, "result"), "frameTree");
        }
        public async Task<System.Drawing.Bitmap> FishingScreenshot(FishingPilot.Scene scene, bool hud) {
            var layout = GetObject(await CommandAsync(_socket,"Page.getLayoutMetrics",null,_timeout.Token).ConfigureAwait(false),"result");
            ...
            if(hud){width=Math.Min(w,h*.5);height=h*.2;x=0;y=h-height;}
                ...
                // NUI resources are full-screen iframes. Do not reuse desktop/DPI coordinates.
                double sx=scene.Width>0?w/scene.Width:1,sy=scene.Height>0?h/scene.Height:1;
                side=Math.Max(scene.W*sx,scene.H*sy)*1.20;
                side=Math.Min(side,Math.Min(w,h));x=scene.X*sx+scene.W*sx/2-side/2;y=scene.Y*sy+scene.H*sy/2-side/2;width=height=side;
            }
            x=Math.Max(0,Math.Min(w-width,x));y=Math.Max(0,Math.Min(h-height,y));
            double scale=hud?900/h:320/width;
            var args=new Dictionary<string,object>{{"format","png"},{"fromSurface",true},{"captureBeyondViewport",false},
                {"clip",new Dictionary<string,object>{{"x",x},{"y",y},{"width",width},{"height",height},{"scale",scale}}}};
            var result=GetObject(await CommandAsync(_socket,"Page.captureScreenshot",args,_timeout.Token).ConfigureAwait(false),"result");
            byte[] bytes=Convert.FromBase64String(GetString(result,"data"));
            using(var stream=new MemoryStream(bytes))using(var bitmap=new System.Drawing.Bitmap(stream))return new System.Drawing.Bitmap(bitmap);
        }
