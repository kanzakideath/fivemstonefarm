...
(() => {
  if (window.__fpProbe3) return window.__fpProbe3.sample(false, null);
  ...
  function readRing(){
    ...
    if(digits.length===1){const d=digits[0];let shape=null;for(let p=d.e.parentElement,n=0;p&&n<5;p=p.parentElement,n++){const b=box(p);if(b.w>40&&b.w<Math.min(W,H)*.65&&b.h>40&&b.h<Math.min(W,H)*.65&&Math.abs(b.w-b.h)<Math.max(b.w,b.h)*.25){shape=b;break;}}
      if(shape)return {valid:false,present:true,key:+d.e.textContent.trim(),stamp:'pixel:'+id(d.e),...shape};}
  ...
  function readAncillary(){
    let hunger=null,water=null;const out=[];
    for(const e of document.querySelectorAll('[role="alert"],[class*="Notification"],[class*="notification"],[class*="toast"]')){
      if(!visible(e)||e.querySelector('[role="alert"],[class*="Notification-root"],[class*="notification-item"]'))continue;const t=(e.innerText||e.textContent||'').trim();if(!t||t.length>400)continue;
  ...
  function keyboard(r,cycle){
    ...
    const changed=state.stamp!==r.stamp||state.key!==r.key||Math.abs(delta(r.start,state.start))>5||Math.abs(r.span-state.span)>5||(state.lastAt>0&&r.pointer<state.lastPointer-45);
    ...
    const release=()=>{if(state.up){clearTimeout(state.up);state.up=null;}target.dispatchEvent(new KeyboardEvent('keyup',init));};
  ...
  const api={
    sample(fast,command){
      ...
      if(!fast||(!r.present&&begin-ancillaryAt>=150)){ancillary=readAncillary();ancillaryAt=begin;}
      return JSON.stringify({w:innerWidth,h:innerHeight,at:begin,ring:r,...ancillary,input,delivery:state.delivery,cost:performance.now()-begin,documentVisible:document.visibilityState,focused:document.hasFocus()});
    },
